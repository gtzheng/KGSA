...
def compute_logits(feat, proto, metric='dot', temp=1.0):
    ...
    if feat.dim() == 2:
        if metric == 'dot':
            ...
            logits = -torch.norm(feat.unsqueeze(1).expand(len(feat), *(proto.shape))-proto.unsqueeze(0).expand(len(feat), *(proto.shape)),p=2, dim=-1)
        ...
        if metric == 'dot':
            ...
            logits = -torch.norm(feat.unsqueeze(2).expand(*(feat.shape[0:2]), *(proto.shape[1:]))-proto.unsqueeze(1).expand(*(feat.shape[0:2]), *(proto.shape[1:])),p=2, dim=-1)
...
def make_optimizer(params, name, lr, gamma=0.1, weight_decay=None, milestones=None):
    if weight_decay is None:
        weight_decay = 0.
    if name == 'sgd':
        optimizer = SGD(params, lr, momentum=0.9, nesterov=True, weight_decay=weight_decay)
    elif name == 'adam':
        optimizer = Adam(params, lr, weight_decay=weight_decay)
    elif name == 'adamw':
        optimizer = torch.optim.AdamW(params, lr, weight_decay=weight_decay)
    elif name == 'vanilla_sgd':
        optimizer = SGD(params, lr, momentum=0.0, weight_decay=weight_decay)
    if milestones:
